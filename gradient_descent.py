...
OUTPUT_DICT_TYPE = {"last": 0, "best":1, "average":2}
...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        #USED: recitation 12 procedure GRADIENT DESCENT(f , {ηt})
        # Initialize x (1) ∈ R^d :
        ...
        cur_delta = np.Inf
        self.callback_(weights=f.weights,val=cur_val,grad=cur_grad,t=t,eta=eta,delta=cur_delta)
        ...
        for t in range(self.max_iter_):
            # CHECK w^(t)-w^(t-1) is more than the specified self.tol_
            if cur_delta <= self.tol_:
                break

            # x (t+1) ← x(t) −ηt∇ fx(t)
            f.weights = last_weights - eta*cur_grad

            # updating values:
            cur_val = f.compute_output(X=X, y=y)
            cur_grad = f.compute_jacobian(X=X, y=y)
            cur_delta = np.linalg.norm(f.weights - last_weights)

            last_weights = f.weights
            sum_weights = sum_weights +  f.weights
            if (best_val >= cur_val):
                best_weights = f.weights
                best_val = cur_val

            self.callback_(weights=f.weights, val=cur_val, grad=cur_grad, t=t+1, eta=eta, delta=cur_delta)

        # end for
        output = [last_weights, best_weights, sum_weights / (t+1)]
        return output[OUTPUT_DICT_TYPE[self.out_type_]]
        # end procedure